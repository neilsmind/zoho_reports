...
describe ZohoReports::Client do
  context "with authentication information" do
    before :each do
  ...
  context "without authentication information" do
    ...
    context "#import_data" do
      ...
      it "should NOT perform any send actions" do

        widget = [ { id: 1, name: 'Acme Widget', description: 'Widget from Acme for Testing', active: 't' } ]
        
        response = @client.import_data("widgets", 'APPEND', widget.to_json)
         expect(WebMock).should_not have_requested(:post, "https://reportsapi.zoho.com/api/user@example.com/test_database/widgets")
      end
    end
  end
end